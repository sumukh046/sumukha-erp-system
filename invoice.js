...
function saveCustomer() {
    ...
    if (editingCustomerIndex === null) {
        ...
        db.customers[editingCustomerIndex] = {
            ...db.customers[editingCustomerIndex],
            name,
            address,
            stateCode,
...
function attachCustomerSelectListener() {
    document.getElementById("customerSelect")?.addEventListener("change", function () {
        ...
        if (this.value !== "") {
            const cust = appCustomers[this.value];
            if (cust) {
                if(phoneField) phoneField.value = cust.phone || "";
                if(billName) billName.value = cust.name;
                if(billAddress) billAddress.value = cust.address;
                if(billState) billState.value = cust.stateCode || cust.state || "";
                
                toggleCustomerLock(true);
            }
        } else {
            if(billName) billName.value = ""; 
            if(billAddress) billAddress.value = ""; 
...
function editCustomer(index) {
    ...
    document.getElementById("customerState").value = cust.stateCode || cust.state || "";
...
const indianStates = [
"Karnataka", "Maharashtra", "Tamil Nadu", "Kerala", "Telangana", "Andhra Pradesh", "Delhi", "Uttar Pradesh", "Gujarat", "Rajasthan", "West Bengal", "Bihar", "Madhya Pradesh", "Punjab", "Haryana", "Odisha", "Assam", "Jharkhand", "Chhattisgarh", "Himachal Pradesh", "Uttarakhand", "Goa", "Tripura", "Manipur", "Meghalaya", "Nagaland", "Mizoram", "Sikkim", "Arunachal Pradesh", "Andaman and Nicobar Islands", "Chandigarh", "Dadra and Nagar Haveli and Daman and Diu", "Lakshadweep", "Puducherry", "Ladakh", "Jammu and Kashmir"
];
...
function filterInvoices() {
    ...
    const taxFilterEl = document.getElementById("invoiceTaxFilter");
    const taxFilter = taxFilterEl ? taxFilterEl.value : "All";
    const searchInput = document.getElementById("invoiceSearchInput");
    const statusFilterEl = document.getElementById("invoiceStatusFilter");
    ...
    appInvoices.forEach((inv, index) => {
        ...
        const matchesTax = taxFilter === "All" || 
                   (taxFilter === "none" && !inv.taxType) ||
                   inv.taxType === taxFilter;

        if (matchesSearch && matchesStatus && matchesTax) {
    ...
    row.innerHTML = `
        ...
        <td>${getTaxLabel(inv.taxType)}</td>
        <td>
            <select class="status-${inv.status}" onchange="updateInvoiceStatus(${index}, this.value, this)">
                <option value="Draft" ${inv.status === "Draft" ? "selected" : ""}>Draft</option>
                <option value="Pending" ${inv.status === "Pending" ? "selected" : ""}>Pending</option>
                <option value="Sent" ${inv.status === "Sent" ? "selected" : ""}>Sent</option>
                <option value="Paid" ${inv.status === "Paid" ? "selected" : ""}>Paid</option>
...
appCustomers = db.customers || [];
    ...
    toggleCustomerLock(true); // Engages the lockdown on load
});